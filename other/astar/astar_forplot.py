...
from pq import Node, Priority_Queue
import math
#### END OF YOUR IMPORTS ####
...
if __name__ == "__main__":
	...
	start = time.clock()
        ...
        def add_to_pq(q, this_p, end_p, grid, x_grid_num, y_grid_num, theta_num, points_plot):
            ...
            if not env.CheckCollision(robot):
                ...
                points_plot.append( (1, [this_node.x, this_node.y,0]) )
                # for i in [-1,0,1]:
                #     for j in [-1,0,1]:
                #         for k in [-1,0,1]:
                ...
                if this_p[0]-1 >= 0:
                    next_node = grid[this_p[0]-1][this_p[1]][this_p[2]]
                    if next_node.parentid == (-1,-1,-1):
                        next_node.parentid = this_node.id
                        q.put((cal_cost(this_node, next_node, end_node), [this_p[0]-1,this_p[1],this_p[2]] ))
                if this_p[0]+1 < x_grid_num:
                    next_node = grid[this_p[0]+1][this_p[1]][this_p[2]]
                    if next_node.parentid == (-1,-1,-1):
                        next_node.parentid = this_node.id
                        q.put((cal_cost(this_node, next_node, end_node), [this_p[0]+1,this_p[1],this_p[2]]))
                if this_p[1]-1 >= 0:
                    next_node = grid[this_p[0]][this_p[1]-1][this_p[2]]
                    if next_node.parentid == (-1,-1,-1):
                        next_node.parentid = this_node.id
                        q.put((cal_cost(this_node, next_node, end_node), [this_p[0],this_p[1]-1,this_p[2]]))
                if this_p[1]+1 < y_grid_num:
                    next_node = grid[this_p[0]][this_p[1]+1][this_p[2]]
                    if next_node.parentid == (-1,-1,-1):
                        next_node.parentid = this_node.id
                        q.put((cal_cost(this_node, next_node, end_node), [this_p[0],this_p[1]+1,this_p[2]]))
                if this_p[2]-1 >= 0:
                    next_node = grid[this_p[0]][this_p[1]][this_p[2]-1]
                    if next_node.parentid == (-1,-1,-1):
                        next_node.parentid = this_node.id
                        q.put((cal_cost(this_node, next_node, end_node), [this_p[0],this_p[1],this_p[2]-1]))
                if this_p[2]+1 < theta_num:
                    next_node = grid[this_p[0]][this_p[1]][this_p[2]+1]
                    if next_node.parentid == (-1,-1,-1):
                        next_node.parentid = this_node.id
                        q.put((cal_cost(this_node, next_node, end_node), [this_p[0],this_p[1],this_p[2]+1]))
        ...
        points_plot = []
        add_to_pq(q, start_p, end_p, grid, x_grid_num, y_grid_num, theta_num, points_plot)
        next_p = q.get()[1]
        while next_p != end_p:
            add_to_pq(q, next_p, end_p, grid, x_grid_num, y_grid_num, theta_num, points_plot)
            next_p = q.get()
            f_cost = next_p[0]
            next_p = next_p[1]
            # print(next_p)
 	...
 	path = [] #put your final path in this variable
        ...
        handles.append(env.drawlinestrip(points=pts,linewidth=3.0,
                                         colors=array(((0,0,0)))))
        for ppl in points_plot:
            if ppl[0] == 1:
                handles.append(env.plot3(points=array( ppl[1]),pointsize=5.0,
                            colors=array((0,0,1))))
            else:
                if not (1, ppl[1]) in points_plot:
                    handles.append(env.plot3(points=array( ppl[1]),pointsize=5.0,
                                colors=array((1,0,0))))
