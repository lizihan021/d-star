...
if __name__ == "__main__":
    ...
    handles = []
    with env:
        path, collisions, all_searched = astar_algorithm.AStar(env, robot, goalconfig, False)
        #### Draw the X and Y components of the configurations explored by your algorithm
        if len(path) == 0:
            print "Failed to find path"
        else:
            path_print = numpy.array(path)
            path_print[:,2] = 0.1
            handles.append(env.plot3(points=numpy.array(collisions),pointsize=15.0,colors=[1,0,0]))
            handles.append(env.plot3(points=numpy.array(all_searched),pointsize=15.0,colors=[0,0,1]))
            handles.append(env.drawlinestrip(points=path_print,linewidth=7.0,colors=[0,0,0]))
        #### END OF YOUR CODE ###
	end = time.clock()
